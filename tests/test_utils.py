...
if config.ROBOT_RADIUS != 0.7:
    pytest.skip(
        "Tests for tesselationLinesBetween() are only valid for robot radius equal to 0.7",
...
def test_tesselation_lines_between():
    ...
    assert [
        ...
        5.3,
        6.7,
    ] == utils.tesselationLinesBetween(-13.1, 8.2, 3)
...
def test_remove_duplicates_int():
    assert [1, 2, 4, 5, 7] == utils.removeDuplicatesPreservingOrder(
        [1, 2, 4, 5, 1, 2, 1, 1, 7]
    )
